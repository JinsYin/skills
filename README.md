...
| [`sdd`](plugins/sdd/) | `claude-local` + `to-requirements` + 4 套 `*-best-practices` + `spec-setup` + `spec-triage` + `product-spec` + `design-to-code` | 规范驱动开发：原始需求 → 立规范 → 定产品功能 → 出设计 → 产出代码 |
...
- `claude-local` - 安装组装式的 Agent 约定规范至项目的 `CLAUDE.local.md` 及 `AGENTS.md` 入口文件
