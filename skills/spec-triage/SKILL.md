...
- **放错层** —— 只在写 Java 时才需要的 DTO 命名表，被放进「写代码之前就必然加载」的位置。实测案例：11 个规则文件的 `paths:` 都带着该项目工作流的产物通配，导致任何规划命令无差别注入约 10.5 KB，且子代理各付一遍。换个工作流只是路径不同，机制一样。
...
- **高频文档与工作流产物路径** —— 本项目有没有在用某种规划/任务工作流（例如 `docs/adr/**`、issue 模板或其他阶段产物），它的产物落在哪；这是 G1 判定的输入，**不要假定，查了再说**
