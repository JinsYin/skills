...
description: Spring Boot 3 + Java 21 + MyBatis Plus 项目的编码规范与架构纪律。涵盖分层依赖、国密 SM2/SM3/SM4、MyBatis Plus 实体与主键策略、多方言数据库迁移、R/RList/RPage 响应包装、DTO 职责边界、鉴权框架选型（Spring Security / Sa-Token）与声明式权限校验、命名约定、模块 README 文档要求。在编写、审查或重构 Spring Boot 后端代码时使用——尤其是新增 Controller/Service/Mapper、定义 Entity 或 DTO、写数据库迁移、处理加解密与签名、设计 API 契约、或接入鉴权与权限校验时。
license: MIT
metadata:
  author: JinsYin
  version: "1.0.0"
...
| 接入鉴权 / 加权限校验 | `stack-auth-framework`、`layer-controller-auth-annotations` |
...
- `layer-controller-auth-annotations` — 权限用 Spring Security / Sa-Token 注解声明，手写 if 判断是默认放行
...
- `stack-auth-framework` — 中大型/基础设施选 Spring Security，中小型/业务系统选 Sa-Token，一个项目只用一套
...
需要一次性获取全部规则时读 `AGENTS.md`（约 50 KB）。该文件由 `scripts/build.sh` 从 `rules/` 生成，**不要手工编辑**。
