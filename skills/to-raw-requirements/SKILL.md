---
name: to-raw-requirements
description: 将用户口述、聊天记录或提供的原始需求与上下文方案，澄清并整理成项目根目录 docs/requirements/raw.md。固定按「项目定位与核心能力、产品交付矩阵、产品功能调研、技术实现参考」四段结构输出；区分需求、约束与参考方案，对未提及、含糊或冲突的信息逐项向用户确认，确认完整后再保存文件。在用户说「整理原始需求」「记录原始需求」「把口述需求落盘」「补全原始需求文档」「to-raw-requirements」时使用，也用于按同一结构更新已有 docs/requirements/raw.md。
---

# to-raw-requirements

将口述需求与上下文方案整理为可追溯的原始需求文档。

## 基本原则

- 把用户原话、当前对话、用户点名的本地材料和已有需求文档视为事实来源。
...
3. 默认目标是 `<项目根目录>/docs/requirements/raw.md`，不得改回根目录 `REQUIREMENTS.md`。
4. 如果根目录已有 `REQUIREMENTS.md`，先询问用户是否按本规范迁移到 `docs/requirements/raw.md`；用户确认后才可移动或删除原文件。用户拒绝时保留原文件不动，并继续处理当前目标文件。
5. 若目标文件已存在，先完整读取并判断用户是补充、重整还是替换。用户意图不明确时，先确认；不得直接覆盖。
...
若目标文件已存在，只在用户确认的范围内修改。需要创建目录时只创建 `docs/requirements/`。完成后重新读取文件，检查实际落盘内容。
...
最后报告保存路径，并简要列出用户明确确认的「无 / 不适用 / 待定」项。然后询问用户是否要提交本次需求文档变更到 Git；只有用户明确同意后才执行提交，不要自动提交。不要在文件之外继续扩写新需求。
